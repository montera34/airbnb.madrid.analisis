...
local_activo_name = "Madrid" #cambia 'Zarautz' por el municipio que quieras analizar
...
# listings_simple <- read.delim("../data/original/insideairbnb_listings_madrid_180117.csv",sep = ",")
listings_simple <- read.delim("../data/output/listings_airbnb-madrid_insideairbnb-datahippo_merged.csv",sep = ",")
# Load reviews
...
+ InsideAirbnb.com/Madrid y Datahippo.org
+ Fecha scraping: 2018-01-17 y 2018-04-16 respectivamente
...
De esos anuncios `r nrow(local_activo[local_activo$withlicense==TRUE,])` han sido dados de alta  (lo que supone un `r format(round(100*nrow(local_activo[local_activo$withlicense==TRUE,])/nrow(local_activo), 1), nsmall = 1)` % del municipio).
...
kable(naccommodates[1:30,],caption = "Los 50 anfitriones (host_id) con más alojamientos tienen n plazas (accomodates)",col.names = c("host_id","plazas"))
```
